...
class _netD(nn.Module):
    def __init__(self, ngpu, num_classes=1, nc=1, ndf=64):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv2d(ndf * 4, num_classes, 4, 2, 1, bias=False),
            #nn.BatchNorm2d(ndf * 8),
            #nn.LeakyReLU(0.2, inplace=True),
...
class dense_D(nn.Module):
    def __init__(self , dim_h, num_classes):
        super(dense_D, self).__init__()
        self.num_classes = num_classes
        self.main = nn.Sequential(
            nn.Linear(784, dim_h * 2),
            #nn.BatchNorm1d(dim_h * 2), better without this
            nn.LeakyReLU(0.2),
            nn.Linear(dim_h * 2, dim_h * 2),
            #nn.BatchNorm1d(dim_h * 2),
            nn.LeakyReLU(0.2),
            nn.Linear(dim_h * 2, dim_h * 2),
            #nn.BatchNorm1d(dim_h * 2),
            nn.LeakyReLU(0.2),
            ...
            #nn.BatchNorm1d(dim_h * 2),
            nn.LeakyReLU(0.2),
            nn.Linear(dim_h * 2, num_classes),
        )
    def forward(self, x):
        z = self.main(x)
        return z

class dense_D_mog(nn.Module):
    def __init__(self , dim_h, num_classes):
        super(dense_D_mog, self).__init__()
        self.num_classes = num_classes
        self.main = nn.Sequential(
            nn.Linear(2, dim_h),
            #nn.BatchNorm1d(dim_h * 2), better without this
