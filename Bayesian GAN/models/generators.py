...
class _netG(nn.Module):
    def __init__(self, ngpu, nz=100, ngf=64, nc=1, batch_norm_layers=[], affine=True):
...
class dense_G(nn.Module):
    def __init__(self, dim_h, nz):
        super(dense_G, self).__init__()
        self.main = nn.Sequential(
            nn.Linear(nz, dim_h * 2),
...
class dense_G_mog(nn.Module):
    def __init__(self, dim_h, nz):
        super(dense_G_mog, self).__init__()
        self.main = nn.Sequential(
            nn.Linear(nz, dim_h),
