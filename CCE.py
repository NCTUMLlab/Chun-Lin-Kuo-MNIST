...
class ComplementCrossEntropyLoss(torch.nn.Module):
  ...
  def forward(self, input, target=None):
    ...
    if target is not None:
      ...
      #print(torch.log(1. - torch.nn.functional.softmax(input)))
    result = torch.nn.functional.nll_loss(
      torch.log(1. - torch.nn.functional.softmax(input , dim = 1) + 1e-4),  #Cuz this is 1-softmax(input),so target is 0
